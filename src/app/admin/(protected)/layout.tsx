...
export default async function ProtectedAdminLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  if (!isSupabaseConfigured()) return <main className="admin-shell"><div className="container narrow"><p className="eyebrow">Admin studio</p><h1>Connect Supabase to activate the backend.</h1><p className="lead-small muted">The public site is running on seeded content. Add the Supabase URL and publishable key, apply the migration, then create the first admin user to manage the live content.</p><Link className="button" href="/">Return to site</Link></div></main>;
